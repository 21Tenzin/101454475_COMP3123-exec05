...
- Why is `express.Router()` used in Express.js applications, and how does it benefit the code structure?
   - `express.Router()` is used in Express.js applications because it separateds the routes into smaller modules and organizes them. It is beneficial to the code structure as it makes the code more readable and scalable.
...
- Explain how the `app.listen(process.env.port || 8081)` line works and why it's useful in production environments.
  - The `app.listen(process.env.port) || 8081` is used to listen for request on the specific port and start the express server. 
    The line is useful in production environment as it can write a code in scalable and flexible manner.
---
